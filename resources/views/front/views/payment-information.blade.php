...
@section('content')
    <main class="flex min-h-screen w-full flex-col items-center">
        <div class="mx-auto max-w-xl space-y-7 rounded-lg bg-white p-4 shadow-md">
            <header class="relative grid grid-cols-[30%_auto] gap-5 rounded-lg bg-yellow-300 px-3 py-2">
                <lottie-player src="{{ asset('lottie/payment-loading.json') }}" background="transparent" speed="0.5"
                    class="sm:h-[150px] sm:w-[150px]" loop autoplay>
                </lottie-player>
                <div class="flex flex-col justify-center">
                    <section>
                        <h1 class="font-bold text-gray-500 sm:text-2xl">Menunggu Pembayaran</h1>
                        <p class="text-sm text-gray-500 sm:text-base">Silahkan lakukan pembayaran berdasarkan metode yang
                            anda pilih</p>
                    </section>
            ...
            <section class="flex flex-col gap-2">
                <h1 class="text-lg font-bold">Detail Tiket</h1>
                <div
                    class="relative mx-auto flex h-auto w-full flex-col justify-between overflow-hidden rounded-xl shadow-sm">
                    <!-- Bagian atas -->
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <p class="text-xs sm:text-base">
                                    {{ \Carbon\Carbon::parse($voluntrip->start_date)->translatedFormat('d F Y') }} -
                                    {{ \Carbon\Carbon::parse($voluntrip->start_date)->translatedFormat('d F Y') }}</p>
                            </div>
                            <div class="flex items-center gap-2">
                                <x-icon.location />
                                <p class="text-xs sm:text-base">{{ $voluntrip->location ?? 'Belum ditentukan' }}</p>
                            </div>
                        </div>
                    ...
                    <div class="relative z-10">
                        <div
                            class="absolute left-0 top-1/2 -ml-3 h-6 w-6 -translate-y-1/2 rounded-full border border-gray-400 bg-white">
                        </div>
                        <hr class="border-1 border-t border-dashed border-black" />
                        <div
                            class="absolute right-0 top-1/2 -mr-3 h-6 w-6 -translate-y-1/2 rounded-full border border-gray-400 bg-white">
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div class="">
                            ...
                            <div class="flex items-center justify-end gap-1">
                                <p class="mb-1 text-sm text-gray-700 sm:text-base">{{ $numVolunteers }}</p>
                                <x-icon.ticket />
                            </div>
                        </div>
                    </div>
            ...
            .then(data => {
                ...
                if (data.status === 'expired') {
                    window.location.href =
                        "{{ route('payment.already_expired', ['payment' => $payment->uuid]) }}"
                }
            });
